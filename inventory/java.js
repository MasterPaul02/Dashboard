function updateProductTable() {
  ...
  existingProducts.forEach((product, index) => {
    ...
    row.insertCell(4).innerHTML = `
      ...
      <button class="btn btn-danger btn-sm" onclick="prepareDelete(${index})">
...
function deleteProduct() {
  ...
  products.splice(deleteIndex, 1); // Remove product from the array
  localStorage.setItem('products', JSON.stringify(products)); // Update local storage
  closeModal(document.getElementById('deleteConfirmationModal')); // Close the modal
